...
export default function SignInPage() {
  ...
  const handleEmail = async () => {
    ...
    try {
      ...
      if (result.status === 'needs_first_factor') {
        ...
        if (f) { await signIn.prepareFirstFactor({ strategy: 'email_code', emailAddressId: (f as any).emailAddressId }); setStep('verify'); }
      }
    } catch (e: any) { setError(e.errors?.[0]?.message || 'Something went wrong'); }
    setLoading(false);
  };
  ...
  const handleVerify = async () => {
    ...
    } catch (e: any) { setError(e.errors?.[0]?.message || 'Invalid code'); }
    setLoading(false);
  };
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', position: 'relative', overflow: 'hidden', background: '#1a0a2e' }}>
      ...
      {/* FIFA 2026 colorful swirl background */}
      <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', zIndex: 0 }}>
        {/* Big color blobs */}
        <div style={{ position: 'absolute', top: '-20%', left: '-20%', width: '70%', height: '70%', borderRadius: '50%', background: 'radial-gradient(circle, #e8003d 0%, #8b0000 50%, transparent 70%)', opacity: 0.7 }}/>
        <div style={{ position: 'absolute', top: '-10%', right: '-20%', width: '60%', height: '60%', borderRadius: '50%', background: 'radial-gradient(circle, #7b2fff 0%, #4a0080 50%, transparent 70%)', opacity: 0.6 }}/>
        <div style={{ position: 'absolute', bottom: '-20%', left: '-10%', width: '65%', height: '65%', borderRadius: '50%', background: 'radial-gradient(circle, #ff5c1a 0%, #8b2500 50%, transparent 70%)', opacity: 0.5 }}/>
        <div style={{ position: 'absolute', bottom: '-15%', right: '-15%', width: '55%', height: '55%', borderRadius: '50%', background: 'radial-gradient(circle, #00c2a8 0%, #006655 50%, transparent 70%)', opacity: 0.5 }}/>
        <div style={{ position: 'absolute', top: '30%', left: '10%', width: '40%', height: '40%', borderRadius: '50%', background: 'radial-gradient(circle, #ffd700 0%, #8b7000 50%, transparent 70%)', opacity: 0.3 }}/>
        <div style={{ position: 'absolute', top: '20%', right: '5%', width: '35%', height: '35%', borderRadius: '50%', background: 'radial-gradient(circle, #00e676 0%, #006633 50%, transparent 70%)', opacity: 0.3 }}/>
      ...
      <div style={{ maxWidth: 400, width: '100%', zIndex: 1 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <h1 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 52, letterSpacing: 6, lineHeight: 1, background: 'linear-gradient(135deg, #ff4466 0%, #ff8c00 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', textShadow: 'none' }}>FANVERSE</h1>
        ...
        <div style={{ background: 'rgba(5, 10, 30, 0.75)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 24, padding: '28px', backdropFilter: 'blur(30px)', WebkitBackdropFilter: 'blur(30px)', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}>
          ...
          <button onClick={signInWithGoogle} disabled={loading} style={{ width: '100%', padding: '13px', borderRadius: 12, border: 'none', cursor: 'pointer', background: 'white', color: '#111', fontWeight: 800, fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 16, boxShadow: '0 4px 20px rgba(0,0,0,0.4)' }}>
          ...
          {step === 'email' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <p style={{ textAlign: 'center', fontSize: 11, color: 'rgba(255,255,255,0.2)' }}>No account? We'll create one automatically.</p>
            </div>
          )}
