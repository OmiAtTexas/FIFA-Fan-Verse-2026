...
export default function FollowingPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/users/${params.id}/following`, { headers: { 'x-user-id': userId || '' } })
    ...
    <div className="page">
      <header className="app-header">
        <div className="app-header-inner" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <h1 className="fifa-font" style={{ fontSize: 22, color: '#e8003d' }}>FOLLOWING</h1>
        </div>
      </header>
      <main className="inner" style={{ paddingBottom: 100 }}>
        ...
        {!loading && users.length === 0 && <div style={{ textAlign: 'center', padding: '60px 20px' }}><p style={{ color: 'var(--text2)' }}>Not following anyone yet</p></div>}
